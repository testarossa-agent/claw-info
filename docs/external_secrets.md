...
以下為將 `ollama-cloud` provider 的 `apiKey` 遷移至 Cloudflare Workers KV 的完整流程。
...
│  ┌────────────────────────────────────────┐                    │
│  │         Secrets 解析引擎               │                    │
│  │  1. 找到 provider: cloudflare_kv       │                    │
│  │  2. 執行 command (exec source)         │                    │
│  └────────────────────┬───────────────────┘                    │
│                       │ spawn                                   │
│                       ▼                                         │
│  ┌────────────────────────────────────────┐                    │
│  │       ~/bin/cf-kv-wrapper.sh           │                    │
│  │  (user-owned, chmod 700)               │                    │
│  │                                        │                    │
│  │  wrangler kv key get                   │                    │
...
└───────────────────────┼─────────────────────────────────────────┘
          ...
          └─────────────────────────────┘
                        ...
                        ▼
┌─────────────────────────────────────────────────────────────────┐
│  cf-kv-wrapper.sh 輸出（exec protocol v1）：                     │
│  {                                                              │
│    "protocolVersion": 1,                                        │
│    "values": { "ollama-cloud-apikey": "sk-..." }               │
│  }                                                              │
...
### 1. 建立 KV Namespace 與 Key

```bash
# 建立 KV namespace（若尚未建立）
wrangler kv namespace create "openclaw"

# 記下輸出中的 namespace ID，例如：
# { binding = "openclaw", id = "abcd1234..." }

# 寫入 secret
wrangler kv key put \
  --namespace-id <KV_NAMESPACE_ID> \
  "ollama-cloud-apikey" "your-api-key-here"
```

> 建議用有意義的 key 名稱統一存放所有 openclaw 相關 key，方便統一管理。
...
#!/bin/bash
# 從環境變數或此處直接設定 KV_NAMESPACE_ID
KV_NAMESPACE_ID="${CF_KV_NAMESPACE_ID}"

# 讀取所有需要的 key
OLLAMA_CLOUD_APIKEY=$(wrangler kv key get \
  --namespace-id "$KV_NAMESPACE_ID" \
  "ollama-cloud-apikey" 2>/dev/null)

python3 -c "
import json, sys
values = {
    'ollama-cloud-apikey': '''${OLLAMA_CLOUD_APIKEY}'''
}
print(json.dumps({'protocolVersion': 1, 'values': values}))
"
EOF
...
```json
{
  "secrets": {
    "providers": {
      "cloudflare_kv": {
        "source": "exec",
        "command": "/home/user/bin/cf-kv-wrapper.sh",
        "timeoutMs": 3000,
        "jsonOnly": false
      }
    }
...
### Cloudflare API Token 權限需求

Wrangler 需要具備以下權限的 API Token（建議使用 `CLOUDFLARE_API_TOKEN` 環境變數）：
...
> ⚠️ 建議透過系統 service 的環境變數或 Kubernetes Secret 注入 `CLOUDFLARE_API_TOKEN`，避免明文寫入設定檔。
