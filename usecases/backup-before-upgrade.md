...
這兩種方式不只適用於備份，是 OpenClaw 定期任務的通用選擇原則：

| | cron | heartbeat |
|---|---|---|
| **適合** | 精準定時 | 帶語境的定期檢查 |
| **觸發條件** | 到點就跑，不判斷 | Agent 評估後決定是否執行 |
| **靈活度** | 低（時間固定） | 高（可加條件判斷） |

以備份為例：
- 每週日 03:00 固定備份 → 用 **cron**
- 「距上次備份超過 7 天才備份」→ 用 **heartbeat**（需要判斷時間間隔）

**一句話選擇原則：** 時間固定用 cron；需要判斷「是否該執行」用 heartbeat。
